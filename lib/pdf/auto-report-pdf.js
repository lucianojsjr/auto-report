...
AutoReportPDF.prototype.init = function(template) {
	...
	this.style = [`${__dirname}/../css/bootstrap.min.css`, `${__dirname}/../css/auto-report.css`];
...
AutoReportPDF.prototype.addStyle = function(callback) {
	async.concat(this.style, (filepath, callback) => fs.readFile(filepath, 'utf-8', callback), (err, styles) => {
		this.template = this.renderer.getTemplate();
		
		const pageElement = cheerio.load(this.template);
		const style = _.join(styles, '');
 ...
 * @param options
 */
AutoReportPDF.prototype.renderChart = function(id, options) {
	const chart = this.renderer.renderChart(id, options);
	
	if (!AutoReportValidator.isChartValid(chart)) {
		return;
	}
	...
	return new Promise((resolve, reject) => {
		async.auto({
			...
			create: ['style', (results, done) => {
				...
				this.report_config.charts = this.charts;
