...
const AutoReportPDFChart = require('./auto-report-pdf-chart');
 ...
 * @param options
 */
AutoReportPDFRenderer.prototype.renderChart = function(tag, options) {
	this.template = this.template.replace(`{{@${tag}}}`, `<div id=${tag}></div>`);
	
	return AutoReportPDFChart.renderChart(tag, options);
};
